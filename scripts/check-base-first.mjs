...
for (const file of walk(sourceRoot).filter((item) => item.endsWith('.jsx'))) {
  ...
  if (!allowedNativeOwners.has(relative)) {
    if (/<(?:form|input|select|textarea|table|thead|tbody|th|td)\b/.test(source)) failures.push(`${relative}: dựng native form/table ngoài base owner`);
    if (/role=["']dialog["']/.test(source)) failures.push(`${relative}: dựng dialog ngoài GamingModal`);
  }
  for (const marker of forbiddenLegacyMarkers) {
    if (source.includes(marker)) failures.push(`${relative}: còn marker legacy ${marker}`);
  }
}
...
console.log('Base-first UI contract passed: feature code reuses canonical form, table, modal, notice, panel, step, list and cell owners.');
