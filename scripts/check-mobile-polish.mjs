...
const transactionDocuments = fs.readFileSync(
    new URL(
...
for (const token of [
    ...
    if (!css.replace(/\s+/g, "").includes(token.replace(/\s+/g, "")))
        failures.push(`Thiếu mobile polish contract: ${token}`);
}
if (!profile.includes('context="account"'))
    failures.push(
        "Profile chưa dùng nhãn trạng thái theo ngữ cảnh khách hàng.",
    );
if (!documents.includes("ResponsiveDataTable"))
    failures.push("Hồ sơ tài liệu chưa dùng bảng canonical.");
if (!transactionDocuments.includes("transaction-documents__attached-list"))
    failures.push(
        "Tài liệu trong chi tiết giao dịch chưa dùng danh sách đính kèm responsive.",
    );
if (failures.length) {
