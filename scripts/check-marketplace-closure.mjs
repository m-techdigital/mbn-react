...
const files = {
    ...
    nav: fs.readFileSync(
        new URL("../src/config/navigation.js", import.meta.url),
        "utf8",
    ),
};
const failures = [];
for (const [name, token] of [
    ...
    ["detail", "TransactionAssetSnapshots"],
    ["nav", "Trung tâm yêu cầu"],
]) {
    if (!Object.values(files).some((value) => value.includes(token)))
        failures.push(`Thiếu ${name}: ${token}`);
}
if (failures.length) {
