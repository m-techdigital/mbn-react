...
function walk(dir) {
    for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
        ...
        if (entry.isDirectory()) walk(full);
            ...
            const tags = [
                ...source.matchAll(/<(input|select|textarea|form)\b/g),
            ].map((match) => match[1]);
            if (tags.length)
                violations.push(`${rel}: raw ${[...new Set(tags)].join(", ")}`);
        }
    }
}
walk(root);
if (violations.length) {
    console.error(
        "Form base contract failed:\n" +
            violations.map((item) => `- ${item}`).join("\n"),
    );
    process.exit(1);
}
console.log(
    "Form base contract passed: all feature forms use canonical base components.",
);
