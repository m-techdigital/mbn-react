...
const failures = [];
if (!profile.includes('loadingVariant="profile"'))
    failures.push("ProfilePage chưa khai báo skeleton hồ sơ.");
if (
    !shell.includes("<AsyncContent") ||
    !shell.includes("loadingVariant={loadingVariant}")
)
    failures.push("PageShell chưa sở hữu AsyncContent/skeleton tải trang.");
if (!profile.includes("mbn-profile-summary"))
