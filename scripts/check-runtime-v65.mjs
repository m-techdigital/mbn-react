...
if (!layout.includes('<RouteMetadata />')) failures.push('RouteMetadata chưa được gắn vào layout.');

if (failures.length) {
  console.error(failures.join('\n'));
  process.exit(1);
}
console.log('Runtime v65 contract OK: cookie-only auth, query cache and route metadata are canonical.');
