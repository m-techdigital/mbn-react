...
import { HistoryOutlined, LoginOutlined, WalletOutlined, CreditCardOutlined } from '@ant-design/icons';
...
const shortcuts = [
  ...
  ['/account/wallet/transactions', CreditCardOutlined, 'Ví khách hàng', 'Biến động số dư'],
];

export default function HomePage() {
  const mockLoadingMs = Number(import.meta.env.VITE_HOME_MOCK_LOADING_MS || 950);
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => setPreparing(false), mockLoadingMs);
    return () => window.clearTimeout(timer);
  }, [mockLoadingMs, shouldFakeLoad]);

  const account = customer || user;
  const serviceItems = [...games, ...services, {
    title: 'Hướng dẫn mua nick',
    path: '/guides/huong-dan-mua-nick',
    image: '/images/banners/history-buy-account-purple-v8.png',
  }];
      ...
      <section className="home-shortcuts">
        {shortcuts.map(([to, Icon, kicker, title]) => (
          <Link to={to} key={to}>
            ...
            <span>
              <small>{kicker}</small>
              <b>{to === '/account/profile' && account ? account.name || account.username : title}</b>
            </span>
          </Link>
        ))}
      </section>
      ...
      <section className="service-section">
        ...
        <div className="legacy-service-grid">
          {serviceItems.slice(0, 8).map((item) => (
            <Link to={item.path} key={`${item.path}-${item.title}`} className="legacy-service-card">
              <MarketplaceImage src={item.image} alt={item.title} />
