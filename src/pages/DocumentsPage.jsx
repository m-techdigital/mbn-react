...
export default function DocumentsPage() {
    ...
    const { data, loading, error, reload } = useRemoteData(
        () => documentRepository.mine(),
        [],
        { queryKey: "documents", staleTime: 30000 },
    );
    const documents = data?.data || data || [];
    const [working, setWorking] = useState("");
    const [preview, setPreview] = useState(null);
    const [agreed, setAgreed] = useState(false);
    const isAccepted = (document) =>
        Boolean(
    ...
    const view = async (document) => {
        ...
        try {
            ...
            setPreview({
                ...result,
                id: document.id,
                accepted_by_current_customer:
                    isAccepted(document) ||
                    result?.accepted_by_current_customer,
            });
            setAgreed(false);
        } catch (errorValue) {
            showToast(
                "error",
                getUserFacingError(errorValue, "Không thể mở tài liệu."),
            );
        } finally {
            setWorking("");
        }
    ...
    const accept = async () => {
        ...
        try {
            await documentRepository.accept(preview.id, {
                accepted_terms: true,
            ...
            showToast(
                "error",
                getUserFacingError(errorValue, "Không thể xác nhận tài liệu."),
            );
        } finally {
            setWorking("");
        }
        ...
        },
        {
            ...
            render: (_, document) => (
                <PrimaryTextCell
                    ...
                    description={`Phiên bản ${document.version}`}
                />
            ),
        },
        {
    ...
    return (
        <PageShell
            ...
            description="Hồ sơ, phụ lục và biên bản được phát hành theo từng giao dịch."
            loading={loading}
            ...
            >
                ...
                {alreadyAccepted ? (
                    <div className="document-accepted-banner">
                        Bạn đã xác nhận tài liệu này. Không cần xác nhận lại.
                    </div>
                ) : (
                    <BaseChoice
