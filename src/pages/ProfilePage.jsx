...
export default function ProfilePage() {
    ...
    const uploadAvatar = async (file) => {
        ...
        try {
            ...
            const { errors } = applyValidationError(
                exception,
                setProfileErrors,
            );
            showToast(
                ...
                Object.keys(errors).length
                    ? "Không thể cập nhật ảnh đại diện. Vui lòng kiểm tra tệp đã chọn."
                    : getUserFacingError(
                          exception,
    ...
    const saveProfile = async (event) => {
        ...
        try {
            ...
            showToast(
                "error",
                Object.keys(errors).length
                    ? "Thông tin cá nhân chưa hợp lệ. Vui lòng kiểm tra các trường được đánh dấu."
                    : getUserFacingError(
                          exception,
    ...
    const requestEmail = async (event) => {
        ...
        setEmailErrors({});
        try {
            await profileRepository.requestEmailChange({ email: newEmail });
            showToast(
                "success",
                "Đã gửi liên kết xác nhận đến địa chỉ thư điện tử mới.",
            );
            setNewEmail("");
        } catch (exception) {
            const { errors } = applyValidationError(exception, setEmailErrors);
            showToast(
                "error",
                Object.keys(errors).length
                    ? "Địa chỉ thư điện tử chưa hợp lệ."
                    : getUserFacingError(
                          exception,
                          "Không thể gửi yêu cầu đổi địa chỉ thư điện tử.",
                      ),
            );
        } finally {
            setBusy("");
    ...
    const changePassword = async (event) => {
        ...
        try {
            ...
            showToast(
                "error",
                Object.keys(errors).length
                    ? "Thông tin mật khẩu chưa hợp lệ. Vui lòng kiểm tra các trường được đánh dấu."
                    : getUserFacingError(exception, "Không thể đổi mật khẩu."),
            );
        } finally {
            setBusy("");
        }
    ...
    return (
        <PageShell
            ...
            description="Quản lý thông tin cá nhân, ảnh đại diện, địa chỉ thư điện tử và mật khẩu."
        ...
        >
            <PageColumns ratio="profile">
                <PageSection
                    ...
                    description="Thông tin hiển thị trên sản phẩm và giao dịch."
                >
                    <div className="mbn-profile-summary">
                        ...
                        <div className="mbn-profile-summary__copy">
                    ...
                    <BaseForm className="mbn-form-stack" onSubmit={saveProfile}>
                        ...
                        <ImageUploadField
                            ...
                            circular
                            error={
                                profileErrors.avatar || profileErrors.avatar_url
                            }
                ...
                <PageStack>
                    <PageSection
                        title="Đổi địa chỉ thư điện tử"
                        description={`Địa chỉ hiện tại: ${customer?.email || "Chưa thiết lập"}`}
                    >
                        <BaseForm
                        ...
                        >
                            <FormField
                                label="Địa chỉ thư điện tử mới"
                                error={emailErrors.email}
                                hint="Địa chỉ mới chỉ được áp dụng sau khi bạn mở liên kết xác nhận trong hộp thư."
                                required
                            >
                                <BaseInput
                                    type="email"
                    ...
                    <PageSection
                        title="Đổi mật khẩu"
                        description="Sử dụng mật khẩu mạnh và không dùng lại mật khẩu của thư điện tử."
                    >
                        <BaseForm
                            className="mbn-form-stack"
