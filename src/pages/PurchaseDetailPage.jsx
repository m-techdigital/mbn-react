    ...
    {transaction ? <PageStack>
      ...
      <PageColumns ratio="wide-left" className="purchase-detail-workspace">
        <PageStack>
          ...
          {transaction.transaction_type === 'rental' && (Number(transaction.rental_deposit_deduction_amount || 0) > 0 || Number(transaction.refunded_amount || 0) > 0) ? <PageSection title="Quyết toán tiền cọc" description="Số tiền cọc được đối chiếu sau hoàn trả. Khấu trừ chỉ áp dụng khi có chi phí hoặc thiệt hại được quản trị viên ghi rõ."><DefinitionGrid items={[{ label: 'Tiền cọc ban đầu', value: formatMoney(transaction.deposit_amount || 0) }, { label: 'Số tiền khấu trừ', value: formatMoney(transaction.rental_deposit_deduction_amount || 0) }, { label: 'Số tiền cọc đã hoàn', value: formatMoney(Math.max(0, Number(transaction.deposit_amount || 0) - Number(transaction.rental_deposit_deduction_amount || 0))) }, { label: 'Căn cứ khấu trừ', value: transaction.rental_deposit_deduction_note || 'Không phát sinh khấu trừ tiền cọc' }]} /></PageSection> : null}
