...
import { marketplaceOperationsRepository, transactionRepository } from '../services/repositories';
...
export default function SupportCasesPage() {
  const [cases, setCases] = useState([]); const [transactions, setTransactions] = useState([]); const [busy, setBusy] = useState(false);
  const [form, setForm] = useState({ transaction_id: '', case_type: 'support', reason: '', description: '', priority: 'normal' });
  const load = async () => {
    const [casePayload, transactionPayload] = await Promise.all([marketplaceOperationsRepository.cases(), transactionRepository.list({ per_page: 100 })]);
    setCases(casePayload?.data || casePayload || []); setTransactions(transactionPayload?.data || transactionPayload || []);
  };
  ...
  const submit = async (event) => {
    ...
    try { await marketplaceOperationsRepository.openCase(form.transaction_id, form); showToast('success', 'Đã gửi yêu cầu hỗ trợ.'); setForm({ transaction_id: '', case_type: 'support', reason: '', description: '', priority: 'normal' }); await load(); }
    catch (error) { showToast('error', getUserFacingError(error, 'Không thể gửi yêu cầu.')); }
