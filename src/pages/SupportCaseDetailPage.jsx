 ...
 return <PageShell title={item?.code||'Chi tiết yêu cầu'} description="Theo dõi nội dung, trạng thái và trao đổi của yêu cầu." loading={loading} width="wide">
  {item?<PageStack>
   ...
   <PageSection title="Trao đổi" description="Chỉ hiển thị nội dung trao đổi dành cho khách hàng.">
    <div className="support-case-thread">{(item.messages||[]).map((entry)=><article key={entry.id} className={`support-case-message is-${entry.actor_type}`}><header><b>{entry.actor_type==='customer'?'Khách hàng':'Bộ phận hỗ trợ'}</b><time>{new Date(entry.created_at).toLocaleString('vi-VN')}</time></header><p>{entry.message}</p></article>)}</div>
    <BaseForm onSubmit={submit}><FormField label="Phản hồi mới" required><BaseTextarea value={message} onChange={e=>setMessage(e.target.value)} required rows={5}/></FormField><BaseFormActions><GamingButton type="submit" loading={busy}>Gửi phản hồi</GamingButton></BaseFormActions></BaseForm>
   </PageSection>
  </PageStack>:null}
 </PageShell>;
}
