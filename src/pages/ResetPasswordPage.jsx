...
export default function ResetPasswordPage() {
  ...
  const submit = async (event) => {
    ...
    const next = {};
    if (form.password.length < 8) next.password = 'Mật khẩu phải có ít nhất 8 ký tự.';
    if (form.password !== form.password_confirmation) next.password_confirmation = 'Mật khẩu nhập lại chưa khớp.';
    setErrors(next); if (Object.keys(next).length) return;
    setBusy(true); setNotice('');
    try { await authRepository.resetPassword(form); setDone(true); setNotice('Mật khẩu đã được đặt lại.'); }
