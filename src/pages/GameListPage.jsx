...
export default function GameListPage() {
    ...
    const { data, loading, error, reload } = useRemoteData(
        () => gameRepository[type].list(apiQuery),
        [type, apiQuery],
        { queryKey: "game-list", staleTime: 30000, keepPreviousData: true },
    );
    const rawItems = arrayData(data);
    const sortedItems = [...rawItems].sort((a, b) => {
        if (params.sort === "price_asc")
            return (
                Number(a.sale_price || a.rental_price || 0) -
                Number(b.sale_price || b.rental_price || 0)
            );
        ...
        if (params.sort === "level_desc")
            return (
                Number(b.level || b.product?.metadata?.level || 0) -
                Number(a.level || a.product?.metadata?.level || 0)
            );
        return (
    ...
    const segment = location.pathname.endsWith("/nick-vip")
        ? "vip"
        : location.pathname.endsWith("/nick-gia-re")
          ? "cheap"
          : null;
    const items =
        segment === "vip"
            ? sortedItems.filter(
                  (item) => Number(item.sale_price || 0) >= 900000,
              )
            : segment === "cheap"
              ? sortedItems.filter(
                    (item) => Number(item.sale_price || 0) < 900000,
    ...
    return (
        <PageShell
            title={
                segment === "vip"
                    ? "Nick Ninja School VIP"
                    : segment === "cheap"
                      ? "Nick Ninja School giá rẻ"
                      : type === "ninjas"
                        ? "Nick Ninja School Online"
                        : type === "dragonBalls"
                          ? "Nick Ngọc Rồng Online"
                          : "Nick Avatar XS Diệu Kỳ"
            }
            loading={loading}
            error={error}
            onReload={reload}
        ...
        >
            ...
            <AccountFilters
                type={type}
                onSubmit={(values) =>
                    setParams({ ...values, page: 1, perPage: 12 })
                }
                resultText={`Tìm thấy ${total} tài khoản phù hợp`}
            />
            <section
                className="catalog-trust-strip"
                aria-label="Thông tin giao dịch"
            >
                <span>
                    ✓ Sản phẩm được quản trị viên kiểm tra trước khi hiển thị
                </span>
                <span>✓ Thanh toán và bàn giao được lưu trong hệ thống</span>
                <span>✓ Có cơ chế khiếu nại và hồ sơ tài liệu</span>
            </section>
            {items.length ? (
                ...
                !loading && (
                    <EmptyState
                        ...
                        description="Hãy thay đổi điều kiện tìm kiếm hoặc quay lại sau khi có sản phẩm mới."
                    />
                )
            )}
            {total > 12 && (
