...
const statement =
    "Tôi đã đọc toàn bộ tài liệu, kiểm tra thông tin giao dịch và đồng ý xác nhận bằng phương thức điện tử trên hệ thống.";
...
export default function TransactionDocuments({
    ...
    const load = useCallback(async () => {
        ...
        try {
            ...
            setNotice(
                getUserFacingError(error, "Không thể tải hồ sơ của giao dịch."),
            );
        } finally {
            setLoading(false);
        }
    ...
    const view = async (document) => {
        ...
        try {
            ...
            showToast(
                "error",
                getUserFacingError(error, "Không thể mở tài liệu."),
            );
        } finally {
            setWorking("");
        }
    ...
    const accept = async (document) => {
        ...
        try {
            await documentRepository.accept(document.id, {
                accepted_terms: true,
            ...
            setPreview((current) => ({
                ...current,
                accepted_by_current_customer: true,
            }));
            setAgreed(false);
            await load();
        } catch (error) {
            showToast(
                "error",
                getUserFacingError(error, "Không thể xác nhận tài liệu."),
            );
        } finally {
            setWorking("");
        }
    ...
    const download = async (item) => {
        ...
        try {
            ...
            showToast(
                "error",
                getUserFacingError(error, "Không thể tải tài liệu."),
            );
        } finally {
            setWorking("");
        }
    ...
    const alreadyAccepted = Boolean(
        preview?.accepted_by_current_customer ||
        currentDocument?.accepted_by_current_customer ||
        currentDocument?.acceptances?.some(
    ...
    return (
        <PageSection
            className="transaction-documents transaction-documents--attached"
            title="Hồ sơ giao dịch"
            description={`Chỉ hiển thị hồ sơ và biên bản được phát hành cho giao dịch ${transactionCode || ""}.`}
            actions={
                <GamingButton
                    size="sm"
                    variant="secondary"
        ...
        >
            {notice ? (
                <p className="transaction-documents__notice">{notice}</p>
            ) : null}
            {loading ? (
                <p className="transaction-documents__loading">
                    Đang tải hồ sơ giao dịch...
                </p>
            ) : null}
            {!loading && !documents.length ? (
                <p className="mbn-empty-inline">
                    Chưa có hồ sơ hoặc biên bản được phát hành cho giao dịch
                    này.
                </p>
            ) : null}
            {!loading && documents.length ? (
                <div className="transaction-documents__attached-list">
                    {documents.map((document) => {
                        const accepted = Boolean(
                            document.accepted_by_current_customer ||
                            document.acceptances?.some(
                                (item) => item.customer_id === customer?.id,
                            ),
                        );
                        return (
                            <article
                                key={document.id}
                                className="transaction-document-card"
                            >
                                <div className="transaction-document-card__content">
                                    ...
                                    <b>
                                        {document.title ||
                                            ...
                                            valueLabel(document.document_type)}
                                    </b>
                                    <small>
                                        {document.code} · Phiên bản{" "}
                                        {document.version}
                                    </small>
                                </div>
                                <div className="transaction-document-card__status">
                                    <b>
                                        {(document.acceptances || []).length}/2
                                        bên
                                    </b>
                                    {accepted ? (
                    ...
                    preview ? (
                        <>
                            ...
                            <GamingButton
                                ...
                                loading={working === `accept-${preview.id}`}
                                onClick={() => accept(preview)}
                            >
                                {alreadyAccepted
                                    ? "Đã xác nhận"
                                    : "Xác nhận điện tử"}
            ...
            >
                <div
                    className="mbn-document-preview"
                    dangerouslySetInnerHTML={{ __html: preview?.html || "" }}
                />
                {alreadyAccepted ? (
                    <div className="document-accepted-banner">
                        Bạn đã xác nhận tài liệu này.
                    </div>
                ) : (
                    <BaseChoice
                        className="document-acceptance"
                        label={statement}
                        checked={agreed}
