...
import { ACCOUNT_NAV_ITEMS, LOGOUT_ICON, PRIMARY_NAV_ITEMS, SUPPORT_NAV_ITEMS, isPrimaryNavActive } from '../../config/navigation';
...
export default function Header({ onAccount }) {
  ...
  useEffect(() => {
    if (!account) { setUnreadCount(0); return undefined; }
    let active = true;
    const load = async () => {
      try {
        const result = await notificationRepository.list({ unread: 1, per_page: 1 });
    ...
    <>
      <header className="site-header">
        <div className="header-brand-row">
          <Link to="/" className="brand" aria-label="MuaBanNick.Pro"><MarketplaceImage src="/images/logo-purple.png" alt="MuaBanNick.Pro" /></Link>
          <div className="header-actions">
            {account ? <NotificationTrigger unreadCount={unreadCount} /> : null}
            <button type="button" className="header-account-pill" onClick={onAccount} aria-label={account ? 'Mở hồ sơ khách hàng' : 'Đăng nhập hoặc đăng ký'}>
              <span className="header-account-avatar">{account?.avatar_url ? <MarketplaceImage src={account.avatar_url} alt="Ảnh đại diện" /> : account ? (account?.name || account?.username || 'K').charAt(0).toUpperCase() : <UserOutlined />}</span>
              <span className="header-account-copy"><b>{account?.name || account?.username || 'Tài khoản'}</b>{account?.code ? <small>{account.code}</small> : null}</span>
              <span className="header-account-caret" aria-hidden="true">⌄</span>
            </button>
            <button type="button" className="mobile-menu-button mobile-only" aria-label="Mở menu" aria-expanded={drawer} onClick={() => setDrawer(true)}><MenuOutlined /></button>
          </div>
        </div>
            ...
            {account ? (
              <div className="mobile-account-section">
                <div className="mobile-account-summary">
                  <span className="header-account-avatar">{account?.avatar_url ? <MarketplaceImage src={account.avatar_url} alt="Ảnh đại diện" /> : (account?.name || account?.username || 'K').charAt(0).toUpperCase()}</span>
                  <span><b>{account?.name || account?.username}</b><small>{account?.code || 'Tài khoản khách hàng'}</small></span>
                </div>
                <nav className="mobile-account-links" aria-label="Chức năng tài khoản"><NavItems items={[...ACCOUNT_NAV_ITEMS, ...SUPPORT_NAV_ITEMS]} path={location.pathname} onNavigate={closeDrawer} /></nav>
                <button type="button" className="mobile-menu-logout" onClick={async () => { closeDrawer(); await logout(); }}><LogoutIcon /><span>Đăng xuất</span></button>
              </div>
            ) : (
