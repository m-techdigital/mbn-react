...
export default function AccountFilters({ type, onSubmit, resultText = "" }) {
    ...
    const fields = useMemo(
        () =>
            type === "ninjas"
                ? [
                      { name: "level", label: "Cấp độ" },
                      { name: "class", label: "Phái" },
                      { name: "server", label: "Máy chủ" },
                  ]
                : type === "dragonBalls"
                  ? [
                        { name: "server", label: "Máy chủ" },
                        { name: "planet", label: "Hành tinh" },
                    ]
                  : [
                        { name: "land", label: "Số ô đất" },
    ...
    return (
        ...
        >
            ...
            <FilterField
                label={type === "avatars" ? "Mã nhân vật" : "Tên nhân vật"}
            >
                <BaseInput
            ...
            {fields.map((field) => (
                <FilterField label={field.label} key={field.name}>
                    ...
                    >
                        ...
                        {(selectOptions[field.name] || []).map((value) => (
