...
export default function TransactionReviewForm({ transactionId, status }) {
    const [form, setForm] = useState({
        rating: 5,
        comment: "",
        criteria: { accuracy: 5, speed: 5, support: 5 },
    });
    ...
    const submit = async (event) => {
        ...
        setBusy(true);
        try {
            await trustRepository.review(transactionId, form);
            showToast("success", "Đã lưu đánh giá giao dịch.");
        } catch (error) {
    ...
    return (
        <PageSection
            ...
            description="Đánh giá chỉ được ghi nhận sau khi giao dịch hoàn tất."
        >
            <BaseForm className="mbn-form-stack" onSubmit={submit}>
                <div className="mbn-base-form__grid mbn-base-form__grid--3">
                    {[
                        ["accuracy", "Đúng mô tả"],
                        ["speed", "Tốc độ"],
                        ["support", "Hỗ trợ"],
