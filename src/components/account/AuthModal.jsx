...
export default function AuthModal({ open, initialMode = "login", onClose }) {
    ...
    const demoAccounts = [
        ...
        [
            "seller",
            "Người bán",
            "Kiểm tra tin chờ duyệt, tin bị từ chối và đơn đã bán",
        ],
        ["renter", "Người thuê", "Kiểm tra giao dịch thuê đang hoạt động"],
        [
            "lessor",
            "Người cho thuê",
            "Kiểm tra bàn giao và hoàn trả tài khoản thuê",
        ],
    ...
    const validate = (kind, values) => {
        ...
        if (!values.username?.trim())
            next.username = "Vui lòng nhập tên đăng nhập hoặc thư điện tử.";
        ...
        if (kind === "register") {
            ...
            if (!/^\S+@\S+\.\S+$/.test(values.email || ""))
                next.email = "Địa chỉ thư điện tử chưa hợp lệ.";
        }
        setErrors(next);
        ...
        try {
            ...
            setError(
                Object.keys(serverErrors).length
                    ? "Thông tin chưa hợp lệ. Vui lòng kiểm tra các trường được đánh dấu."
                    : supportCode && status >= 500
                      ? `${messageText} Mã hỗ trợ: ${supportCode}`
                      : messageText,
            );
        } finally {
            setBusy(false);
    ...
    const submitTwoFactor = async (event) => {
        ...
        if (!twoFactor.code.trim())
            return setErrors({ code: "Vui lòng nhập mã xác thực." });
        setBusy(true);
        setError("");
        setErrors({});
        try {
    ...
    return (
        ...
        >
            {error ? (
                <InlineNotice type="error" title="Không thể xác thực">
                    {error}
                </InlineNotice>
            ) : null}
            {twoFactor.required ? (
                <BaseForm
                    className="auth-gaming-form"
                    onSubmit={submitTwoFactor}
                >
                    <InlineNotice>
                        Nhập mã 6 số từ ứng dụng xác thực hoặc một mã khôi phục.
                    </InlineNotice>
                    <FormField label="Mã xác thực" required error={errors.code}>
                        <BaseInput
                ...
                <BaseForm
                    className="auth-gaming-form auth-gaming-form--register"
                    onSubmit={(event) => {
                        event.preventDefault();
                        submit("register", registerData);
                    }}
                >
                    <FormField
                        label="Tên hiển thị"
