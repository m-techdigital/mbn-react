  ...
  return (
    <article className="account-card">
      <Link to={`${basePath}/${code}`} className="account-image">
        <AsyncImage src={imageOf(product)} alt={`Mã ${code}`} loading="lazy" decoding="async" />
      ...
      <div className="account-card-body">
        <div className="account-description">{item.description || product.description || product.name || 'Tài khoản game đã kiểm tra thông tin'}</div>
        ...
        <div className="account-footer">
          <div className="account-price">
            ...
            <span>
              {discount > 0 && oldPrice ? <del>{formatMoney(oldPrice)}</del> : null}
              <b>{formatMoney(currentPrice)}</b>
            </span>
          </div>
          <Link className="view-account-button" to={`${basePath}/${code}`}><span>Xem tài khoản</span><ArrowRightOutlined /></Link>
        </div>
      </div>
    </article>
  );
