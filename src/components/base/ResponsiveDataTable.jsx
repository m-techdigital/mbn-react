...
export default function ResponsiveDataTable({
    ...
    return (
        <div
            className={`mbn-table-shell mbn-semantic-table-shell ${className}`.trim()}
        >
            {caption ? (
                <p className="mbn-table-shell__caption">{caption}</p>
            ) : null}
            {rows.length ? (
                <>
                    <div className="mbn-table-scroll-hint" aria-hidden="true">
                ...
                <EmptyState
                    compact
                    title={emptyText}
                    description="Dữ liệu mới sẽ xuất hiện tại đây khi có phát sinh."
                />
            )}
        </div>
    );
