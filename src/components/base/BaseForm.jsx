export default function BaseForm({
    ...
    return (
        <form
            className={`mbn-base-form ${className}`.trim()}
            onSubmit={onSubmit}
...
export function BaseFormGrid({ children, className = "", columns = 2 }) {
    return (
        <div
            className={`mbn-base-form__grid mbn-base-form__grid--${columns} ${className}`.trim()}
        >
            {children}
        </div>
    );
        ...
        <section className={`mbn-base-form__section ${className}`.trim()}>
            {title || description || actions ? (
                <header className="mbn-base-form__section-head">
                    ...
                    {actions ? (
                        <div className="mbn-base-form__section-actions">
                            {actions}
                        </div>
                    ) : null}
                </header>
...
export function BaseFormActions({ children, className = "", align = "end" }) {
    return (
        <div
            className={`mbn-base-form__actions is-${align} ${className}`.trim()}
        >
            {children}
        </div>
    );
