...
import { fetchQuery, getQueryState, setQueryData, stableSerialize } from '../services/queryClient';
...
export function useRemoteData(loader, dependencies = [], options = {}) {
  ...
  const dependencyKey = useMemo(() => stableSerialize(dependencies), [dependencies]);
  const resolvedQueryKey = useMemo(
    () => queryKey ? `${queryKey}:${dependencyKey}` : null,
    [queryKey, dependencyKey],
  );
  const initial = resolvedQueryKey ? getQueryState(resolvedQueryKey)?.data ?? null : null;
  ...
  const run = useCallback(async (force = false) => {
    ...
    try {
      const result = resolvedQueryKey
        ? await fetchQuery({ key: resolvedQueryKey, loader: () => loaderRef.current(), staleTime, force })
  ...
  useEffect(() => {
    const cached = resolvedQueryKey ? getQueryState(resolvedQueryKey)?.data : null;
    if (cached != null) {
      setDataState(cached);
      setLoading(false);
    }
  ...
  return { data, loading, error, reload, setData, queryKey: resolvedQueryKey };
}
