...
const RULE_TRANSLATIONS = [
  [/required/i, 'Vui lòng nhập :field.'], [/must be a valid email/i, ':field chưa đúng định dạng.'], [/has already been taken/i, ':field đã được sử dụng.'],
  [/must be at least (\d+)/i, ':field phải có ít nhất $1 ký tự.'], [/must not be greater than (\d+)/i, ':field không được vượt quá $1 ký tự.'],
  [/confirmation does not match/i, ':field chưa khớp.'], [/must be a number|must be numeric/i, ':field phải là số hợp lệ.'],
  [/must be an image/i, ':field phải là tệp ảnh hợp lệ.'], [/failed to upload/i, 'Không thể tải :field lên.'],
];

const cleanKey = (key='') => String(key).replace(/\.\d+(?=\.|$)/g, '').replace(/^data\./, '');
export const fieldLabel = (key) => FIELD_LABELS[cleanKey(key)] || cleanKey(key).split('.').pop().replaceAll('_', ' ');
...
export function translateValidationMessage(message, key='') {
  ...
  for (const [pattern, replacement] of RULE_TRANSLATIONS) {
    if (pattern.test(text)) return replacement.replace(':field', fieldLabel(key).toLowerCase()).replace(/^./, c => c.toUpperCase());
  }
  return text;
}
...
export function validationErrorsFrom(error) {
  return normalizeValidationErrors(error?.validationErrors || error?.response?.data?.errors || {});
}

export function applyValidationError(error, setErrors, fallbackMessage) {
  const errors = validationErrorsFrom(error);
  setErrors?.(errors);
  return { errors, message: fallbackMessage || 'Thông tin nhập vào chưa hợp lệ. Vui lòng kiểm tra các trường được đánh dấu.' };
}
