...
const containsTechnicalText = (value = '') => TECHNICAL_PATTERNS.some((pattern) => pattern.test(String(value)));
...
const TRANSLATED_MESSAGES = [
  ...
  [/refresh token expired/i, 'Phiên đăng nhập đã hết hạn. Vui lòng đăng nhập lại.'],
  [/not found/i, 'Nội dung bạn tìm kiếm hiện không tồn tại hoặc chưa sẵn sàng.'],
  [/too many requests/i, 'Bạn thao tác quá nhanh. Vui lòng chờ một lát rồi thử lại.'],
];
...
export function getUserFacingError(error, fallback = 'Không thể xử lý yêu cầu lúc này. Vui lòng thử lại sau.') {
  const status = Number(error?.response?.status || error?.status || 0);
  const validationMessage = Object.values(error?.validationErrors || error?.response?.data?.errors || {}).flat().find(Boolean);
  if (validationMessage && !containsTechnicalText(validationMessage)) return translateKnownMessage(validationMessage);
  ...
  const safeCandidate = candidates.find((message) => !containsTechnicalText(message));
...
export function getSupportCode(error) {
  return error?.requestId || error?.correlationId || error?.response?.headers?.['x-request-id'] || null;
}
